...
export function evaluateReadiness(f: ReadinessFacts): ReadinessResult {
  ...
  // ---- Communication ------------------------------------------------------------------------
  add({ id: "email", category: "Communication", title: "Email provider configured", required: true, status: f.comms.email ? "PASS" : "BLOCKED", detail: f.comms.email ? "SMTP credentials present." : "No email provider — messages are logged, not delivered.", remediation: "Set SMTP_USER and SMTP_PASS (and optionally EMAIL_FROM) in Vercel." });
