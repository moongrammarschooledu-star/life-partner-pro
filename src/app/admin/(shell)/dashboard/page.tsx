  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-primary" /> Trust &amp; Verification
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
            {[
              { label: "Awaiting Verification", value: data.trustKpis.awaitingVerification },
              { label: "Verified", value: data.trustKpis.verified },
              { label: "Verification Required", value: data.trustKpis.verificationRequired },
              { label: "Rejected", value: data.trustKpis.rejected },
              { label: "Re-Verification Required", value: data.trustKpis.reVerificationRequired },
              { label: "Suspended", value: data.trustKpis.suspended },
              { label: "Potential Duplicates", value: data.trustKpis.potentialDuplicates },
            ].map((k) => (
              <div key={k.label} className="rounded-lg border border-border p-3 text-center">
                <p className="text-xl font-semibold">{k.value}</p>
                <p className="mt-1 text-xs text-muted">{k.label}</p>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div>
              <p className="mb-2 text-sm font-medium">Verification Status</p>
              <BarChart data={data.verificationByStatus.map((v) => ({ label: v.label.replaceAll("_", " "), count: v.count }))} title="" />
            </div>
          </div>
          <Link href="/admin/verification" className={buttonClass({ variant: "outline", className: "mt-4" })}>
