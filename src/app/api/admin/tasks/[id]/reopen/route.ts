...
import { reopenTask } from "@/lib/workflow/engine";

export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const admin = await requireAdmin("tasks:reopen");
    ...
    const updated = await reopenTask({ taskId: id, actorId: admin.id, reason: reasonText });
