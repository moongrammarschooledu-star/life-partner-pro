...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      payments,
      communications: { email: Boolean(process.env.SMTP_USER?.trim() && process.env.SMTP_PASS?.trim()), sms: Boolean(process.env.SMS_PROVIDER_API_KEY), whatsappEnabled: process.env.WHATSAPP_ENABLED === "true", failedDeliveries24h: commFailures.map((c) => ({ channel: c.channel, count: c._count._all })) },
